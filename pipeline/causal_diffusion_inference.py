...
class CausalDiffusionInferencePipeline(torch.nn.Module):
        ...
        for current_num_frames in all_num_frames:
            ...
            for _, t in enumerate(sample_scheduler.timesteps):
                ...
                temp_x0 = sample_scheduler.step(
                    ...
                    return_dict=False)[0]
                latents = temp_x0
                _debug_print(f"kv_cache['local_end_index']: {self.kv_cache_pos[0]['local_end_index']}")
                _debug_print(f"kv_cache['global_end_index']: {self.kv_cache_pos[0]['global_end_index']}")
    ...
    def _reset_cache_entries(self, caches, device):
        """
        Reset all per-prompt KV state while keeping allocated tensors reusable.
        """
        tensor_zero_keys = (
            "k",
            "v",
        ...
        tensor_fill_neg_one_keys = ("abs_frame_idx", "spatial_idx")
        transient_keys = (
            "history_k",
            "history_v",
        ...
        for cache in caches:
            for key in tensor_zero_keys:
                value = cache.get(key, None)
                if torch.is_tensor(value):
                    value.zero_()
            for key in tensor_fill_neg_one_keys:
                value = cache.get(key, None)
                if torch.is_tensor(value):
                    value.fill_(-1)
            ...
            cache["ema_initial"] = False

            q_ready = cache.get("q_calib_ready", None)
            if torch.is_tensor(q_ready):
                q_ready.fill_(False)
            elif "q_calib_ready" in cache:
                cache["q_calib_ready"] = None
    ...
    def _reset_kv_cache(self, device):
        self._reset_cache_entries(self.kv_cache_pos, device)
        self._reset_cache_entries(self.kv_cache_neg, device)
